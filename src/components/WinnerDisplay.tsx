...
export default function WinnerDisplay({
  ...
  useEffect(() => {
    if (!isRaffling || participants.length === 0 || visualMode !== "slots")
      return;
  ...
  }, [isRaffling, participants, visualMode]);
  ...
  if (winnerRevealed && winner) {
    return (
      <div
        ...
        style={{
          background:
            "linear-gradient(150deg, #0d3d7a 0%, var(--color-brand-blue) 50%, #0d3d7a 100%)",
        }}
      >
        {/* Confetti */}
        <div
        ...
        <div className="relative z-10 px-6 sm:px-8 py-10 w-full max-w-lg mx-auto flex flex-col items-center">
          ...
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-3xl p-6 sm:p-8 w-full shadow-xl relative">
            ...
            <div className="mt-8 flex items-center justify-center gap-2 border-t border-white/10 pt-6">
              ...
              <p className="text-lg font-bold text-brand-white">{prize}</p>
            </div>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          backgroundImage:
            "radial-gradient(circle at 2px 2px, var(--color-brand-blue) 1px, transparent 0)",
      ...
      {visualMode === "slots" && (
        <div className="flex flex-col items-center justify-center px-6 py-12 z-10 w-full overflow-hidden">
            ...
            <div className="flex flex-col items-center text-center gap-5 animate-fade-in-up">
              ...
              <div>
                ...
                <p className="text-brand-mauve/80 text-sm md:text-base max-w-xs mx-auto leading-relaxed">
                  Sube tu listado de participantes y define el premio en el
                  panel izquierdo para empezar.
                </p>
              </div>
          ...
          {/* Idle: only 1 participant */}
          {participants.length === 1 && !isRaffling && !current && (
            <div className="flex flex-col items-center text-center gap-5 animate-fade-in-up">
              <div className="w-20 h-20 rounded-full bg-blue-50 border border-blue-200 flex items-center justify-center shadow-sm">
                <FiUsers className="text-3xl text-brand-blue" />
              </div>
              <div>
                <h3 className="text-2xl lg:text-3xl font-black text-brand-text tracking-tight mb-3">Necesitas más participantes</h3>
                <p className="text-brand-mauve/80 text-sm md:text-base max-w-xs mx-auto leading-relaxed">
                  Se necesitan al menos <span className="font-black text-brand-blue">2 participantes</span> para realizar un sorteo.
                </p>
              </div>
            </div>
          )}
          ...
          {participants.length > 1 &&
            ...
            !current && (
              <div className="flex flex-col items-center text-center gap-5 animate-fade-in-up">
                ...
                <div>
                  <h3 className="text-3xl lg:text-4xl font-black text-brand-text tracking-tight mb-3">
                    Todo listo
                  </h3>
                ...
                <div className="mt-1 bg-brand-blue/6 border border-brand-blue/15 rounded-2xl px-5 py-3 flex items-center gap-3">
                  ...
                  <p className="text-sm font-semibold text-brand-text truncate max-w-50">
                    {prize}
                  </p>
                </div>
              </div>
      ...
      {/* ── ROULETTE mode ── */}
      {visualMode === "roulette" && (
        <div className="flex flex-col items-center justify-center px-4 py-8 z-10 w-full overflow-hidden gap-4">
          {/* Idle: no participants */}
          {participants.length === 0 && !isRaffling && (
            <div className="flex flex-col items-center text-center gap-5 animate-fade-in-up">
              <div className="w-20 h-20 rounded-full bg-brand-blue/8 border border-brand-blue/15 flex items-center justify-center shadow-sm">
          ...
          {/* Idle: only 1 participant */}
          {participants.length === 1 && !isRaffling && (
            <div className="flex flex-col items-center text-center gap-5 animate-fade-in-up">
              <div className="w-20 h-20 rounded-full bg-blue-50 border border-blue-200 flex items-center justify-center shadow-sm">
                <FiUsers className="text-3xl text-brand-blue" />
              </div>
          ...
          {participants.length > 1 && (
            <>
              ...
              {!isRaffling && !prize.trim() && !winner && participants.length > 1 && (
                <p className="text-sm text-brand-mauve/80 font-medium animate-fade-in-up text-center">
                  Solo falta definir el premio para iniciar
                </p>
              )}

              {!isRaffling && prize.trim() && !winner && participants.length > 1 && (
                <div className="flex items-center gap-2 bg-brand-blue/6 border border-brand-blue/15 rounded-2xl px-5 py-3 animate-fade-in-up">
                  <FiGift className="text-brand-blue text-lg shrink-0" />
                  <p className="text-sm font-semibold text-brand-text truncate max-w-50">
                    {prize}
                  </p>
